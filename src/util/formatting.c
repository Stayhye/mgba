...
#if defined(HAVE_LOCALE) && !defined(_SYS__LOCALE_H_)
    locale_t l = newlocale(LC_NUMERIC_MASK, "C", 0);
    int res = ftostr_l(str, size, f, l);
    freelocale(l);
    return res;
#else
    // On PS2/newlib without full xlocale support, default to standard formatting
    return snprintf(str, size, "%.*g", FLT_DIG, f);
#endif
}
...
float strtof_u(const char* restrict str, char** restrict end) {
    ...
    return res;
#else
    // On PS2/newlib without full xlocale support, fallback to standard strtof
    return strtof(str, end);
#endif
}
